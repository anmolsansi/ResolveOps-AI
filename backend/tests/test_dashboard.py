...
def test_quality_empty(client, auth_headers):
    resp = client.get("/dashboard/quality", headers=auth_headers)
...
def test_quality_after_upload(client, auth_headers):
    _upload(client, [VALID_ROW], auth_headers)
    resp = client.get("/dashboard/quality", headers=auth_headers)
    assert resp.status_code == 200
    data = resp.json()
...
def test_retrieval_empty(client, auth_headers):
...
def test_retrieval_after_query(client, auth_headers):
    _upload(client, [VALID_ROW], auth_headers)
    client.post("/rag/query", json={"question": "Login problem"}, headers=auth_headers)
    resp = client.get("/dashboard/retrieval", headers=auth_headers)
    assert resp.status_code == 200
    data = resp.json()
...
def test_quality_rates(client, auth_headers):
    rows = [
        VALID_ROW,
...
def test_quality_duplicate_count(client, auth_headers):
...
def test_quality_recent_batches(client, auth_headers):
    _upload(client, [VALID_ROW], auth_headers)
    _upload(client, [{**VALID_ROW, "id": "T-2"}], auth_headers)
    resp = client.get("/dashboard/quality", headers=auth_headers)
    assert resp.status_code == 200
    data = resp.json()
...
def test_retrieval_recent_queries(client, auth_headers):
    _upload(client, [VALID_ROW], auth_headers)
    client.post("/rag/query", json={"question": "Q1"}, headers=auth_headers)
    client.post("/rag/query", json={"question": "Q2"}, headers=auth_headers)
    resp = client.get("/dashboard/retrieval", headers=auth_headers)
    assert resp.status_code == 200
    data = resp.json()
...
def test_retrieval_citation_rate(client, auth_headers):
    _upload(client, [VALID_ROW], auth_headers)
    client.post("/rag/query", json={"question": "test query"}, headers=auth_headers)
    resp = client.get("/dashboard/retrieval", headers=auth_headers)
    assert resp.status_code == 200
    data = resp.json()
